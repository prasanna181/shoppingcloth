...
const Update = () => {
  ...
  const apiCall = async () => {
    console.log("hello from update admin api");
    try {
      await axios
        .post("http://localhost:8000/updateclothitem", newItem)
        .then((res) => {
  ...
  const handleImageChange = (event) => {
    ...
    setSelectedImages((prevNames) => [...prevNames, ...fileNamesArray]);
  };

  useEffect(() => {
    if (productId.length !== 0) apiCall();
  }, [newItem]);
